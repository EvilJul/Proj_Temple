...
/*DIY修改位置1====================================================================================*/
static const uint16_t GATTS_SERVICE_UUID_TEST = 0x181A;   // 环境传感器
static const uint16_t GATTS_CHAR_UUID_CH1     = 0x2A6E;   // 特征1 温度UUID
static const uint16_t GATTS_CHAR_UUID_CH2     = 0x2A6F;   // 特征2 湿度UUID
static const uint16_t GATTS_CHAR_UUID_CH3     = 0x2A57;   // 特征3 数字输出UUID
/*DIY修改位置1====================================================================================*/
...
// char3值
static char sv1_char3_value[1] = {0x00};
...
static esp_ble_adv_params_t adv_params = {
    ...
    .adv_filter_policy =
        ADV_FILTER_ALLOW_SCAN_ANY_CON_ANY,   // 广播过滤策略（接收任何scan和任何连接）
};
    ...
    [SV1_CH1_IDX_CHAR] =
        {
            {ESP_GATT_AUTO_RSP},
            {
                ...
                sizeof(uint8_t),
                sizeof(uint8_t),
                (uint8_t*)&char_prop_read_notify,   // 特征值2只允许读和通知
            },
        },
    ...
    [SV1_CH1_IDX_CHAR_VAL] =
        {
            ...
            {
                ESP_UUID_LEN_16,
                (uint8_t*)&GATTS_CHAR_UUID_CH1,
                ESP_GATT_PERM_READ,
                sizeof(sv1_char1_value),
                sizeof(sv1_char1_value),
                (uint8_t*)sv1_char1_value,
            },
        },
    ...
    [SV1_CH1_IDX_CHAR_CFG] =
        {
            {ESP_GATT_AUTO_RSP},
            {
                ...
                sizeof(uint16_t),
                sizeof(sv1_ch1_client_cfg),
                (uint8_t*)sv1_ch1_client_cfg,
            },
        },
    ...
    [SV1_CH2_IDX_CHAR_VAL] =
        {
            ...
            {
                ESP_UUID_LEN_16,
                (uint8_t*)&GATTS_CHAR_UUID_CH2,
                ESP_GATT_PERM_READ,
                sizeof(sv1_char2_value),
                sizeof(sv1_char2_value),
                (uint8_t*)sv1_char2_value,
            },
        },
    ...
    [SV1_CH2_IDX_CHAR_CFG] =
        {
            {ESP_GATT_AUTO_RSP},
            {
                ...
                ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                sizeof(uint16_t),
                sizeof(sv1_ch2_client_cfg),
                (uint8_t*)&sv1_ch2_client_cfg,
            },
        },
    ...
    [SV1_CH3_IDX_CHAR] =
        {
            {ESP_GATT_AUTO_RSP},
            {
                ...
                sizeof(uint8_t),
                sizeof(uint8_t),
                (uint8_t*)&char_prop_read_write_notify,   // 特征值3只允许读写和通知
            },
        },
    ...
    [SV1_CH3_IDX_CHAR_VAL] =
        {
            ...
            {
                ESP_UUID_LEN_16,
                (uint8_t*)&GATTS_CHAR_UUID_CH3,
                ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                sizeof(sv1_char3_value),
                sizeof(sv1_char3_value),
                (uint8_t*)sv1_char3_value,
            },
        },
    ...
    [SV1_CH3_IDX_CHAR_CFG] =
        {
            {ESP_GATT_AUTO_RSP},
            {
                ...
                ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                sizeof(uint16_t),
                sizeof(sv1_ch3_client_cfg),
                (uint8_t*)&sv1_ch3_client_cfg,
            },
        },
};
...
static uint8_t adv_service_uuid16[] = {
    ...
    0x00,
    0x00,
    GATTS_SERVICE_UUID_TEST & 0xff,
    (GATTS_SERVICE_UUID_TEST >> 8) & 0xff,
    0x00,
    0x00,
...
static esp_ble_adv_data_t adv_data = {
    ...
    .flag                = (ESP_BLE_ADV_FLAG_GEN_DISC |
             ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),   // 普通发现模式|不支持EDR经典蓝牙
};
                                        ...
                                        esp_ble_gatts_cb_param_t* param)
{
    ...
    case ESP_GATTS_REG_EVT:   // 可以认为是一个Profile GATTS启动时会进入此事件
    {
        ...
        esp_err_t create_attr_ret =
            esp_ble_gatts_create_attr_tab(gatt1_db, gatts_if, SV1_IDX_NB, SVC_IND_ID1);
        if (create_attr_ret) {
    ...
    case ESP_GATTS_READ_EVT:   // 收到客户端的读取事件
    {
        ...
        //----------------------------------------------------------------------------------------------
        if (sv1_handle_table[SV1_CH1_IDX_CHAR_VAL] == param->read.handle) {   // 特征值1 温度
            rsp.attr_value.len = sizeof(sv1_char1_value);
            memcpy(rsp.attr_value.value, sv1_char1_value, sizeof(sv1_char1_value));
        }
        if (sv1_handle_table[SV1_CH2_IDX_CHAR_VAL] == param->read.handle) {   // 特征值1 湿度
            rsp.attr_value.len = sizeof(sv1_char2_value);
            memcpy(rsp.attr_value.value, sv1_char2_value, sizeof(sv1_char2_value));
        }
        if (sv1_handle_table[SV1_CH3_IDX_CHAR_VAL] == param->read.handle) {   // 特征值1 LED状态值
            rsp.attr_value.len = sizeof(sv1_char3_value);
            memcpy(rsp.attr_value.value, sv1_char3_value, sizeof(sv1_char3_value));
        }
        //----------------------------------------------------------------------------------------------
    ...
    case ESP_GATTS_WRITE_EVT:   // 收到客户端的写事件
        if (!param->write.is_prep) {
            ...
            if (param->write.handle ==
                sv1_handle_table[SV1_CH1_IDX_CHAR_CFG])   // 特征1客户端特征配置
            {
                sv1_ch1_client_cfg[0] = param->write.value[0];
                sv1_ch1_client_cfg[1] = param->write.value[1];
            }
            else if (param->write.handle ==
                     sv1_handle_table[SV1_CH2_IDX_CHAR_CFG])   // 特征2客户端特征配置
            {
                sv1_ch2_client_cfg[0] = param->write.value[0];
                sv1_ch2_client_cfg[1] = param->write.value[1];
            }
            else if (param->write.handle ==
                     sv1_handle_table[SV1_CH3_IDX_CHAR_CFG])   // 特征3客户端特征配置
            {
                sv1_ch3_client_cfg[0] = param->write.value[0];
                sv1_ch3_client_cfg[1] = param->write.value[1];
            }
            else if (param->write.handle == sv1_handle_table[SV1_CH3_IDX_CHAR_VAL])   // LED的值
            {
                sv1_char3_value[0] = param->write.value[0];
                gpio_set_level(GPIO_NUM_12, sv1_char3_value[0]);
            }
    ...
    case ESP_GATTS_EXEC_WRITE_EVT:
        ...
        break;
    case ESP_GATTS_MTU_EVT: ESP_LOGI(TAG, "ESP_GATTS_MTU_EVT, MTU %d", param->mtu.mtu); break;
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:   // 注册ATTR表成功事件
        if (param->add_attr_tab.status != ESP_GATT_OK) {
            ESP_LOGE(TAG,
                     "create attribute table failed, svc id = %d,error code=0x%x",
...
esp_err_t ble_cfg_net_init(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙控制器men释放失败！");
        return err;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙控制器初始化失败！");
        return err;
    }

    err = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙控制器开启失败！");
        return err;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙广播初始化配置失败！");
        return err;
    }
    err = esp_bluedroid_enable();
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙控制器开启广播失败！");
        return err;
    }

    err = esp_ble_gatts_register_callback(gatts_profile_event_handler);
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙GATT注册回调函数失败！");
        return err;
    }

    err = esp_ble_gap_register_callback(gap_event_handler);
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙GAP注册回调函数失败！");
        return err;
    }

    err = esp_ble_gatts_app_register(ESP_APP_ID);   // 一个APP对应一份Profile
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙GATT应用注册失败！");
        return err;
    }

    err = esp_ble_gatt_set_local_mtu(500);
    if (err != ESP_OK) {
        ESP_LOGE("BLE_INFO", "蓝牙GATT设置本地MTU失败！");
        return err;
    }
    return err;
}
...
void ble_set_ch1_value(uint16_t value)
{
    ...
    if (gl_conn_id != 0xFFFF && (sv1_ch1_client_cfg[0] | sv1_ch1_client_cfg[1])) {
        esp_ble_gatts_set_attr_value(
            sv1_handle_table[SV1_CH1_IDX_CHAR_VAL], 2, (const uint8_t*)&sv1_char2_value);
        esp_ble_gatts_send_indicate(gl_gatts_if,
                                    gl_conn_id,
                                    sv1_handle_table[SV1_CH1_IDX_CHAR_VAL],
                                    2,
                                    (uint8_t*)&sv1_char2_value,
                                    false);
    }
...
void ble_set_ch2_value(uint16_t value)
{
    ...
    if (gl_conn_id != 0xFFFF && (sv1_ch2_client_cfg[0] | sv1_ch2_client_cfg[1])) {
        esp_ble_gatts_set_attr_value(
            sv1_handle_table[SV1_CH2_IDX_CHAR_VAL], 2, (const uint8_t*)&sv1_char2_value);
        esp_ble_gatts_send_indicate(gl_gatts_if,
                                    gl_conn_id,
                                    sv1_handle_table[SV1_CH2_IDX_CHAR_VAL],
                                    2,
                                    (uint8_t*)&sv1_char2_value,
                                    false);
    }
